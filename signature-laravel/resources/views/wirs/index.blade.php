<x-app-layout>
    ...
    <div x-show="filterSidebarOpen" x-cloak class="position-fixed top-0 end-0 h-100 bg-white shadow-lg filter-sidebar" style="z-index: 1050; overflow-y: auto; border-left: 1px solid #e5e7eb;" @click.stop>
        <div class="p-4">
            ...
            <form method="GET" action="{{ route('wirs.index') }}" id="wirFilterForm">
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select name="sort" class="form-select" style="border-radius: 8px; border: 1px solid #e5e7eb;" onchange="document.getElementById('wirFilterForm').submit();">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('wirs.index') }}" class="btn btn-outline-secondary"><i class="fas fa-redo me-2"></i>Reset</a>
                </div>
            </form>
        </div>
    </div>

    <div class="row g-4" x-data="wirApp()">
                ...
                <div class="card-body p-4">
                    <div class="d-flex align-items-center mb-4 pb-3 border-bottom" style="border-color: color-mix(in srgb, var(--primary-color) 20%, transparent) !important;">
                        <div class="rounded-circle bg-primary d-flex align-items-center justify-content-center me-3" style="width: 48px; height: 48px; background: linear-gradient(45deg, var(--primary-color), var(--primary-light)) !important;">
                            <i class="fas text-white" :class="isEditing ? 'fa-edit' : 'fa-file-invoice'"></i>
                        </div>
                        <h2 class="h5 fw-semibold mb-0" style="color: #1f2937;" x-text="isEditing ? 'Edit WIR' : 'Add WIR'"></h2>
                    </div>

                    <!-- ADD FORM -->
                    <div x-show="!isEditing">
                        <form action="{{ route('wirs.store') }}" method="POST">
                            @csrf
                            ...
                            <button type="submit" class="btn btn-primary w-100 py-2 fw-medium">
                                <i class="fas fa-plus me-2"></i>Add WIR
                            </button>
                        </form>
                    </div>
                    ...
                    <div x-show="isEditing" x-cloak>
                        <form :action="`{{ url('wirs') }}/${editingWir.id}`" method="POST">
                            @csrf
                            @method('PUT')

                            <div class="mb-3">
                                <label class="form-label fw-medium" style="color: #374151;">Name</label>
                                <input type="text" name="name" x-model="editingWir.name" class="form-control" required
                                       placeholder="Enter name"
                                       style="border-radius: 8px; border: 1px solid #e5e7eb;">
                            </div>
                    ...
                    <div class="list-header d-flex flex-column flex-lg-row align-items-stretch align-items-lg-center justify-content-between py-3 px-3 px-md-4 border-bottom gap-2" style="border-color: color-mix(in srgb, var(--primary-color) 20%, transparent) !important;">
                        <div class="list-header-title-row d-flex align-items-center justify-content-between flex-shrink-0" style="min-width: 0;">
                            <div class="d-flex align-items-center overflow-hidden" style="min-width: 0;">
                                ...
                                <h2 class="h5 h6 mb-0 fw-semibold text-truncate" style="color: #1f2937;" title="WIR List">WIR List</h2>
                                <span class="badge ms-2 ms-sm-3 flex-shrink-0" style="background-color: color-mix(in srgb, #ef4444 15%, #ffffff); color: #dc2626; font-size: 0.75rem; padding: 0.25rem 0.5rem;">{{ $wirs->total() }} Total</span>
                            </div>
                            <div class="d-flex align-items-center gap-1 ms-2 flex-shrink-0">
                                ...
                                @if(request()->hasAny(['search', 'sort']) && (request('search') || (request('sort') && request('sort') != 'name_asc')))
                                    <a href="{{ route('wirs.index') }}" class="btn btn-outline-secondary d-flex align-items-center justify-content-center" style="border-radius: 8px; width: 36px; height: 36px; min-width: 36px;" title="Clear Filters"><i class="fas fa-times"></i></a>
                                @endif
                            </div>
                        </div>
                        <form method="GET" action="{{ route('wirs.index') }}" class="d-flex align-items-center gap-2 list-header-search flex-grow-1 flex-lg-grow-0" style="min-width: 0;">
                            ...
                            <tbody>
                                @forelse($wirs as $wir)
                                    <tr class="border-bottom" style="transition: all 0.2s ease;">
                                        <td class="px-2"><span class="fw-medium" style="color: #1f2937;">{{ $wirs->firstItem() + $loop->index }}</span></td>
                                        <td class="px-2">
                                            <div class="d-flex align-items-center">
                                                <div class="rounded-circle bg-primary text-white d-flex align-items-center justify-content-center me-3 shadow-sm" 
                                                    ...
                                                    {{ strtoupper(substr($wir->name, 0, 1)) }}
                                                </div>
                                                <div>
                                                    <div class="fw-medium" style="color: #1f2937; font-size: 0.95rem;">{{ $wir->name }}</div>
                                                    <small class="text-muted" style="font-size: 0.75rem;">ID: {{ $wir->id }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-2">
                                            @if($wir->machineCategories->count() > 0)
                                                <div class="d-flex flex-wrap gap-1">
                                                    @foreach($wir->machineCategories as $category)
                                        ...
                                        <td class="px-2">
                                            <div class="d-flex gap-2" role="group">
                                                <button type="button" 
                                                        @click="editWir({
                                                            id: {{ $wir->id }},
                                                            name: '{{ addslashes($wir->name) }}',
                                                            categories: @js($wir->machineCategories)
                                                        })"
...
>
                                                ...
                                                <form action="{{ route('wirs.destroy', $wir) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this WIR?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" 
                                    ...
                                    <tr>
                                        <td colspan="4" class="text-center text-muted py-5">
                                            <div class="d-flex flex-column align-items-center">
                                                ...
                                                <p class="mb-0" style="font-size: 0.9rem;">No WIRs found.</p>
                                                <small class="text-muted mt-1">Add your first WIR to get started</small>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
            ...
            function wirApp() {
                return {
                    ...
                    editWir(wir) {
                        ...
                        this.selectedCategories = Array.isArray(wir.categories)
                            ? wir.categories.map(c => String(c.id)).filter(Boolean)
                            : [];
