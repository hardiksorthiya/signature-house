<x-app-layout>
    <div x-data="preErectionSearch()" x-init="init()">
        <div class="mb-4">
            <div class="row g-3 align-items-center mb-3">
                <div class="col-12 col-lg-auto">
                    <h1 class="h2 fw-semibold mb-1" style="color: #1f2937;">Pre Erection</h1>
                    <p class="text-muted mb-0 small">Track pre-erection status for proforma invoices.</p>
                </div>
            </div>
        </div>
        ...
        {{-- Dedicated search section (PI / Contract dropdown + Search / Reset) --}}
        <div class="card shadow-sm border-0 mb-4" style="background: linear-gradient(to bottom, #ffffff 0%, color-mix(in srgb, var(--primary-color) 6%, #ffffff) 100%); border-radius: 12px;">
            ...
            <div class="card-body p-4">
                ...
                <div class="d-flex gap-2 mt-3">
                    ...
                    <a href="{{ route('pre-erection.index') }}" class="btn btn-outline-secondary d-flex align-items-center" style="border-radius: 8px;">
        ...
        {{-- Same list-card + header pattern as PI List (proforma-invoices/index) --}}
                ...
                <div class="list-header d-flex flex-column flex-lg-row align-items-stretch align-items-lg-center justify-content-between py-3 px-3 px-md-4 border-bottom gap-2" style="border-color: color-mix(in srgb, var(--primary-color) 20%, transparent) !important;">
                    <div class="list-header-title-row d-flex align-items-center justify-content-between flex-grow-1" style="min-width: 0;">
                        <div class="d-flex align-items-center overflow-hidden" style="min-width: 0;">
                            ...
                            <h2 class="h5 h6 mb-0 fw-semibold text-truncate" style="color: #1f2937;" title="Pre-erection list">Pre-erection list</h2>
                        ...
                        @if (request()->filled('pi_number') || request()->filled('contract_number') || request()->filled('search'))
                            <div class="d-flex align-items-center gap-1 ms-2 flex-shrink-0">
                                <a href="{{ route('pre-erection.index') }}" class="btn btn-outline-secondary d-flex align-items-center justify-content-center" style="border-radius: 8px; width: 36px; height: 36px; min-width: 36px;" title="Clear search"><i class="fas fa-times"></i></a>
                            </div>
                        @endif
                    </div>
                </div>
                        ...
                        <tbody>
                            @forelse ($proformaInvoices as $pi)
                                @php
                                    ...
                                    $details = $pi->preErectionDetails;
                                    $hasDetails = $details->isNotEmpty();
                                    $specTotal = count($preErectionSpecLabels);
                                    $completedSpecs = 0;
                                    foreach ($preErectionSpecLabels as $specLabel) {
                                        $row = $details->firstWhere('technical_specification', $specLabel);
                                ...
                                <tr class="border-bottom">
                                    ...
                                    <td class="px-2">
                                        <div class="fw-medium" style="color: #1f2937;">{{ $companyName }}</div>
                                    </td>
                                    <td class="px-2">
                                        <div style="color: #6b7280;">{{ $customerName }}</div>
                                    </td>
                                    <td class="px-2">
                                        <div style="color: #6b7280;">{{ $contract?->contract_number ?? 'N/A' }}</div>
                                    </td>
                                    <td class="px-2">
                                        <div style="color: #6b7280;">{{ $salesManager }}</div>
                                    </td>
                                    <td class="px-2">
                                        <div class="d-flex flex-column align-items-start gap-1">
                                            <span class="fw-semibold" style="color: #1f2937; font-size: 0.95rem; font-variant-numeric: tabular-nums;">{{ $completedSpecs }}/{{ $specTotal }}</span>
                                            @if (!$hasDetails)
                                                <span class="badge bg-warning text-dark">Pending</span>
                                            @elseif ($completedSpecs === $specTotal && $specTotal > 0)
                                                <span class="badge bg-success">Completed</span>
                                            @else
                                                <span class="badge" style="background: color-mix(in srgb, var(--primary-color) 20%, #e0e7ff); color: var(--primary-dark);">In progress</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-2">
                                        <div class="d-flex gap-2" role="group">
                                            <a href="{{ route('pre-erection.show', $pi) }}"
                                               class="btn btn-sm btn-outline-info"
                                               title="{{ $hasDetails ? 'View' : 'Add' }}">
                                                <i class="fas {{ $hasDetails ? 'fa-eye' : 'fa-plus' }}"></i>
                                            </a>
                                        </div>
                                    </td>
                                </tr>
        ...
        function preErectionSearch() {
            return {
                ...
                loadData() {
                    ...
                    fetch(`{{ route('pre-erection.unified-items') }}?q=${q}`, { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
                ...
                submitForm() {
                    const base = @json(route('pre-erection.index'));
