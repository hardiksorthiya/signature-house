<x-app-layout>
    ...
    <div class="mb-4 d-flex justify-content-between align-items-center">
        <div>
            <h1 class="h2 fw-semibold mb-1" style="color: #1f2937;">Brand Management</h1>
            <p class="text-muted mb-0">Manage brands and their related categories</p>
        </div>
    </div>
    ...
    <div x-show="filterSidebarOpen" x-cloak class="position-fixed top-0 end-0 h-100 bg-white shadow-lg filter-sidebar" style="z-index: 1050; overflow-y: auto; border-left: 1px solid #e5e7eb;" @click.stop>
        <div class="p-4">
            ...
            <form method="GET" action="{{ route('brands.index') }}" id="brandFilterForm">
                <div class="mb-3">
                    <label class="form-label fw-medium" style="color: #374151;">Search</label>
                    <input type="text" name="search" value="{{ request('search') }}" class="form-control" placeholder="Search by brand name..." style="border-radius: 8px; border: 1px solid #e5e7eb;">
                </div>
                <div class="mb-3">
                    <label class="form-label fw-medium" style="color: #374151;">Sort By</label>
                    <select name="sort" class="form-select" style="border-radius: 8px; border: 1px solid #e5e7eb;" onchange="document.getElementById('brandFilterForm').submit();">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('brands.index') }}" class="btn btn-outline-secondary"><i class="fas fa-redo me-2"></i>Reset</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="row g-4" x-data="{ 
        ...
        editBrand(brand) {
            ...
            this.selectedCategories = Array.isArray(brand.categories)
                ? brand.categories.map(c => String(c.id ?? c.pivot?.machine_category_id)).filter(Boolean)
                : [];
        ...
        <!-- Left Side: Add/Edit Brand Form (30%) -->
                ...
                <div class="card-body p-4">
                    <div class="d-flex align-items-center mb-4 pb-3 border-bottom" style="border-color: color-mix(in srgb, var(--primary-color) 20%, transparent) !important;">
                        <div class="rounded-circle bg-primary d-flex align-items-center justify-content-center me-3" style="width: 48px; height: 48px; background: linear-gradient(45deg, var(--primary-color), var(--primary-light)) !important;">
                            <i class="fas text-white" :class="isEditing ? 'fa-edit' : 'fa-tags'"></i>
                        </div>
                        <h2 class="h5 fw-semibold mb-0" style="color: #1f2937;" x-text="isEditing ? 'Edit Brand' : 'Add Brand'"></h2>
                    </div>
                    ...
                    <div x-show="!isEditing">
                        <form action="{{ route('brands.store') }}" method="POST">
                            @csrf
                            <div class="mb-4">
                                <label class="form-label fw-medium" style="color: #374151;">Brand Name</label>
                    ...
                    <div x-show="isEditing" x-cloak>
                        <template x-if="editingBrand">
                            <form :action="`{{ url('brands') }}/${editingBrand.id}`" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="mb-4">
                                    <label class="form-label fw-medium" style="color: #374151;">Brand Name</label>
                                    <input type="text" name="name" required
                                           x-model="editingBrand.name"
                                           class="form-control"
                                           placeholder="Enter brand name"
                                           style="border-radius: 8px; border: 1px solid #e5e7eb;">
                                </div>
        ...
        <!-- Right Side: Brand List Table (70%) -->
                    ...
                    <div class="list-header d-flex flex-column flex-lg-row align-items-stretch align-items-lg-center justify-content-between py-3 px-3 px-md-4 border-bottom gap-2" style="border-color: color-mix(in srgb, var(--primary-color) 20%, transparent) !important;">
                        <div class="list-header-title-row d-flex align-items-center justify-content-between flex-shrink-0" style="min-width: 0;">
                            <div class="d-flex align-items-center overflow-hidden" style="min-width: 0;">
                                ...
                                <h2 class="h5 h6 mb-0 fw-semibold text-truncate" style="color: #1f2937;" title="Brand List">Brand List</h2>
                                <span class="badge ms-2 ms-sm-3 flex-shrink-0" style="background-color: color-mix(in srgb, #ef4444 15%, #ffffff); color: #dc2626; font-size: 0.75rem; padding: 0.25rem 0.5rem;">{{ $brands->total() }} Total</span>
                            </div>
                            <div class="d-flex align-items-center gap-1 ms-2 flex-shrink-0">
                                ...
                                @if(request()->hasAny(['search', 'sort']) && (request('search') || (request('sort') && request('sort') != 'name_asc')))
                                    <a href="{{ route('brands.index') }}" class="btn btn-outline-secondary d-flex align-items-center justify-content-center" style="border-radius: 8px; width: 36px; height: 36px; min-width: 36px;" title="Clear Filters"><i class="fas fa-times"></i></a>
                                @endif
                            </div>
                        </div>
                        <form method="GET" action="{{ route('brands.index') }}" class="d-flex align-items-center gap-2 list-header-search flex-grow-1 flex-lg-grow-0" style="min-width: 0;">
                            <div class="flex-grow-1" style="min-width: 0;">
                                <input type="text" name="search" value="{{ request('search') }}" class="form-control form-control-sm" placeholder="Search by brand name..." style="border-radius: 8px; border: 1px solid #e5e7eb; height: 38px;">
                            </div>
                        ...
                        <table class="table table-hover mb-0 align-middle">
                             <thead class="table-head-hp" style="background: linear-gradient(to right, color-mix(in srgb, var(--primary-color) 12%, #ffffff), color-mix(in srgb, var(--primary-color) 18%, #ffffff)) !important;">
                                <tr>
                                    ...
                                    <th class="p-2 small fw-semibold" style="color: var(--primary-color) !important;">Brand Name</th>
                            ...
                            <tbody>
                                @forelse($brands as $brand)
                                    <tr class="border-bottom" style="transition: all 0.2s ease;">
                                        <td class="px-2"><span class="fw-medium" style="color: #1f2937;">{{ $brands->firstItem() + $loop->index }}</span></td>
                                        <td class="px-2">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div>
                                                    <div class="fw-medium" style="color: #1f2937; font-size: 0.95rem;">{{ $brand->name }}</div>
                                                    <small class="text-muted" style="font-size: 0.75rem;">ID: {{ $brand->id }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-2">
                                            @if($brand->machineCategories->count() > 0)
                                                <div class="d-flex flex-wrap gap-1">
                                                    @foreach($brand->machineCategories as $category)
                                            ...
                                            <div class="d-flex gap-2" role="group">
                                                <button type="button" 
                                                        @click="editBrand({
                                                            ...
                                                            categories: @js($brand->machineCategories)
                                                        })"
                                    ...
                                    <tr>
                                        <td colspan="4" class="text-center text-muted py-5">
                                            <div class="d-flex flex-column align-items-center">
                                                <i class="fas fa-tags fa-3x mb-3" style="color: #d1d5db; opacity: 0.5;"></i>
                                                <p class="mb-0" style="font-size: 0.9rem;">No brands found.</p>
                                                <small class="text-muted mt-1">Add your first brand to get started</small>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
