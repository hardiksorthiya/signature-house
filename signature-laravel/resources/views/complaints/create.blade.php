<x-app-layout>
    <div class="mb-4 d-flex flex-column flex-md-row justify-content-between align-items-stretch align-items-md-center gap-2">
        <div>
            <h1 class="h2 fw-semibold mb-1" style="color: #1f2937;">Create Complain</h1>
            <p class="text-muted mb-0">Add a new complaint</p>
        </div>
    ...
    <div class="card shadow-sm border-0" style="background: linear-gradient(to bottom, #ffffff 0%, color-mix(in srgb, var(--primary-color) 6%, #ffffff) 100%); border-radius: 12px;">
        ...
        <div class="card-body p-4" x-data="{
            ...
            clientSearch: '',
            selectedContractId: '{{ old("contract_id", $preselectedContractId ?? "") }}',
            ...
            selectedMachineCategoryId: '{{ old('machine_category_id') }}',
                ...
                <div class="row g-4">
                    <div class="col-12">
                        ...
                        <div class="position-relative" @click.away="clientDropdownOpen = false">
                            <button type="button"
                                    ...
                                    :class="{ 'border-primary': clientDropdownOpen }"
                                    class="form-control text-start d-flex justify-content-between align-items-center @error('contract_id') is-invalid @enderror"
                            ...
                            <div x-show="clientDropdownOpen"
                                ...
                                <div class="p-2 border-bottom" style="border-color: #e5e7eb !important;">
                                    <input type="text"
                                           ...
                                           @click.stop
                                           placeholder="Search by company, buyer name or contract..."
                                ...
                                <template x-for="c in filteredClients" :key="c.id">
                                    <div class="d-flex align-items-center py-2 px-3"
                                        ...
                                        <div class="flex-grow-1">
                                            <div class="fw-medium" x-text="(c.company_name || c.buyer_name) || '—'"></div>
                                            <small class="d-block" x-text="c.contract_number || ''"></small>
                                        </div>
                                    </div>
                                </template>
                            </div>
                    ...
                    <div class="col-12">
                        ...
                        <select name="machine_category_id"
                                x-model="selectedMachineCategoryId"
                                :disabled="!selectedContractId || loadingMachineCategories"
                                class="form-select @error('machine_category_id') is-invalid @enderror"
                                style="border-radius: 8px; border: 1px solid #e5e7eb;">
                            <option value="">— Select category —</option>
                            <template x-for="cat in machineCategoriesForContract" :key="cat.id">
                                <option :value="cat.id" x-text="cat.name"></option>
                            </template>
                        </select>
                        <template x-if="selectedContractId && !loadingMachineCategories && machineCategoriesForContract.length === 0">
                            <div class="small text-muted mt-1">No machine categories for this client.</div>
                        </template>
                        <template x-if="loadingMachineCategories">
                            <div class="small text-muted mt-1"><i class="fas fa-spinner fa-spin me-1"></i>Loading...</div>
                        </template>
                    ...
                    <div class="col-12">
                        ...
                        <select name="complain_type_id" id="complain_type_id" required class="form-select @error('complain_type_id') is-invalid @enderror" style="border-radius: 8px; border: 1px solid #e5e7eb;">
                            ...
                            @foreach($complainTypes as $type)
                                <option value="{{ $type->id }}" {{ old('complain_type_id') == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                            @endforeach
                        </select>
                        @error('complain_type_id')
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" name="machine_khata_number" id="machine_khata_number" value="{{ old('machine_khata_number') }}"
                               class="form-control @error('machine_khata_number') is-invalid @enderror"
                               placeholder="Enter machine khata number or serial number"
                               style="border-radius: 8px; border: 1px solid #e5e7eb;">
                        @error('machine_khata_number')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="other_detail" id="other_detail" rows="4"
                                  class="form-control @error('other_detail') is-invalid @enderror"
                                  placeholder="Enter other details (optional)"
                                  style="border-radius: 8px; border: 1px solid #e5e7eb;">{{ old('other_detail') }}</textarea>
                        @error('other_detail')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        <div class="d-flex flex-wrap gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-2"></i>Create Complain
                            </button>
