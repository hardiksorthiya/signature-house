<x-app-layout>
    ...
    <div class="mb-4">
        <div class="row g-3 align-items-center mb-3">
            <div class="col-12 col-lg">
                <h1 class="h2 fw-semibold mb-1" style="color: #1f2937;">Edit Role</h1>
                <p class="text-muted mb-0 small">Update role and permissions</p>
            </div>
            <div class="col-12 col-lg-auto">
                <div class="d-flex flex-wrap gap-2 justify-content-start justify-content-lg-end">
                    <a href="{{ route('roles.create') }}" class="btn btn-outline-secondary d-flex align-items-center" style="border-radius: 8px;">
                        <i class="fas fa-arrow-left me-1 me-sm-2"></i>Back to Create
                    </a>
                </div>
            </div>
        </div>
    ...
    <div x-show="filterSidebarOpen" x-cloak class="position-fixed top-0 end-0 h-100 bg-white shadow-lg filter-sidebar" style="z-index: 1050; overflow-y: auto; border-left: 1px solid #e5e7eb;" @click.stop>
        <div class="p-4">
            ...
            <form method="GET" action="{{ route('roles.edit', $role) }}" id="roleFilterFormEdit">
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select name="sort" class="form-select" style="border-radius: 8px; border: 1px solid #e5e7eb;" onchange="document.getElementById('roleFilterFormEdit').submit();">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('roles.edit', $role) }}" class="btn btn-outline-secondary"><i class="fas fa-redo me-2"></i>Reset</a>
                </div>
            </form>
        </div>
    </div>
            ...
            <div class="card shadow-sm border-0 h-100" style="background: linear-gradient(to bottom, #ffffff 0%, color-mix(in srgb, var(--primary-color) 6%, #ffffff) 100%);">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center mb-4 pb-3 border-bottom" style="border-color: color-mix(in srgb, var(--primary-color) 20%, transparent) !important;">
                        ...
                        <h2 class="h5 fw-semibold mb-0" style="color: #1f2937;">Edit Role: {{ $role->name }}</h2>
                    </div>
                        ...
                        <div class="list-header-title-row d-flex align-items-center justify-content-between flex-shrink-0" style="min-width: 0;">
                            ...
                            <div class="d-flex align-items-center gap-1 ms-2 flex-shrink-0">
                                ...
                                @if(request()->hasAny(['search', 'sort']) && (request('search') || (request('sort') && request('sort') != 'name_asc')))
                                    <a href="{{ route('roles.edit', $role) }}" class="btn btn-outline-secondary d-flex align-items-center justify-content-center" style="border-radius: 8px; width: 36px; height: 36px; min-width: 36px;" title="Clear Filters"><i class="fas fa-times"></i></a>
                                @endif
                            </div>
                        </div>
                        <form method="GET" action="{{ route('roles.edit', $role) }}" class="d-flex align-items-center gap-2 list-header-search flex-grow-1 flex-lg-grow-0" style="min-width: 0;">
                ...
                <div class="card-body p-0">
                    <div class="table-responsive" style="  overflow-y: auto; overflow-x: hidden;">
                        <table class="table table-hover mb-0 align-middle">
                            ...
                            <tbody>
                                @forelse($roles as $r)
                                    <tr class="border-bottom {{ $r->id == $role->id ? 'table-active' : '' }}" style="transition: all 0.2s ease;">
                                        ...
                                        <td class="px-3 py-2">
                                            <div class="fw-medium" style="color: #1f2937; font-size: 0.9rem;">{{ $r->name }}</div>
                                        </td>
                                        <td class="px-3 py-2">
                                            <small class="text-muted">{{ $r->permissions->count() }} permission(s)</small>
                                        </td>
                                        <td class="px-3 py-2">
                                            <div class="d-flex gap-1 justify-content-center" role="group">
                                                <a href="{{ route('roles.edit', $r) }}" class="btn btn-sm btn-outline-info" title="Edit Role"><i class="fas fa-edit"></i></a>
