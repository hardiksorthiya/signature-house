<x-app-layout>
    <div x-data="damageDetailsSearch()" x-init="init()">
        <div class="mb-4">
            <div class="row g-3 align-items-center mb-3">
                <div class="col-12 col-lg-auto">
                    <h1 class="h2 fw-semibold mb-1" style="color: #1f2937;">Damage Details</h1>
                    <p class="text-muted mb-0 small">View and manage damage details for proforma invoices.</p>
                </div>
        ...
        <div class="card shadow-sm border-0 mb-4" style="background: linear-gradient(to bottom, #ffffff 0%, color-mix(in srgb, var(--primary-color) 6%, #ffffff) 100%); border-radius: 12px;">
            ...
            <div class="card-body p-4">
                ...
                <div class="d-flex flex-wrap gap-2 mt-3">
                    ...
                    <a href="{{ route('damage-details.index') }}" class="btn btn-outline-secondary d-flex align-items-center" style="border-radius: 8px;">
                ...
                <div class="list-header d-flex flex-column flex-lg-row align-items-stretch align-items-lg-center justify-content-between py-3 px-3 px-md-4 border-bottom gap-2" style="border-color: color-mix(in srgb, var(--primary-color) 20%, transparent) !important;">
                    <div class="list-header-title-row d-flex align-items-center justify-content-between flex-grow-1" style="min-width: 0;">
                        <div class="d-flex align-items-center overflow-hidden" style="min-width: 0;">
                            ...
                            <h2 class="h5 h6 mb-0 fw-semibold text-truncate" style="color: #1f2937;" title="Damage details list">Damage details list</h2>
                        ...
                        @if (request()->filled('pi_number') || request()->filled('contract_number') || request()->filled('search'))
                            <div class="d-flex align-items-center gap-1 ms-2 flex-shrink-0">
                                <a href="{{ route('damage-details.index') }}" class="btn btn-outline-secondary d-flex align-items-center justify-content-center" style="border-radius: 8px; width: 36px; height: 36px; min-width: 36px;" title="Clear search"><i class="fas fa-times"></i></a>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <table class="table table-hover mb-0 align-middle">
                        <thead class="table-head-hp" style="background: linear-gradient(to right, color-mix(in srgb, var(--primary-color) 12%, #ffffff), color-mix(in srgb, var(--primary-color) 18%, #ffffff)) !important;">
                            <tr>
                                ...
                                <th class="p-2 small fw-semibold" style="color: var(--primary-color) !important;">Damage entries</th>
                        ...
                        <tbody>
                            @forelse ($proformaInvoices as $pi)
                                @php
                                    ...
                                    $damageCount = $pi->damageDetails ? $pi->damageDetails->count() : 0;
                                @endphp
                                <tr class="border-bottom">
                                    ...
                                    <td class="px-2">
                                        @if ($damageCount > 0)
                                            <span class="badge bg-warning text-dark">{{ $damageCount }} entries</span>
                                        @else
                                            <span class="badge bg-secondary">No entries</span>
                                        @endif
                                    </td>
                                    <td class="px-2">
                                        <div class="d-flex gap-2" role="group">
                                            @canany(['create damage', 'edit damage'])
                                                <a href="{{ route('damage-details.show', $pi) }}"
                                                   class="btn btn-sm {{ $damageCount > 0 ? 'btn-outline-info' : 'btn-success' }}"
                                                   title="{{ $damageCount > 0 ? 'View / add' : 'Add' }} damage details">
                                ...
                                <tr>
                                    <td colspan="8" class="text-center text-muted py-5">
                                        <div class="d-flex flex-column align-items-center">
                                            <i class="fas fa-exclamation-triangle fa-3x mb-3" style="color: #d1d5db; opacity: 0.5;"></i>
        ...
        function damageDetailsSearch() {
            return {
                ...
                loadData() {
                    ...
                    fetch(`{{ route('damage-details.unified-items') }}?q=${q}`, { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
                ...
                submitForm() {
                    const base = @json(route('damage-details.index'));
