    ...
    <div class="container">
        ...
        <div class="header-section">
            <div class="logo-container">
                @if($seller && ($seller->logo ?? $seller->signature))
                    <img src="{{ Storage::url($seller->logo ?? $seller->signature) }}" alt="{{ $seller->seller_name ?? 'Logo' }}" />
                @else
        ...
        <div class="product-section">
            @php
                ...
                $gstAmount = 0;
                $finalAmount = 0;
                $tplPdfIsLocal = ($proformaInvoice->type_of_sale === 'local');
                $tplPdfLocalMult = $tplPdfIsLocal
                    ? (floatval($proformaInvoice->usd_rate ?? 0) > 0 ? floatval($proformaInvoice->usd_rate) : 1.0)
                    : 1.0;
                $tplPdfPrefix = $tplPdfIsLocal ? '₹' : ($proformaInvoice->currency ?? 'USD');
            @endphp
                ...
                <tbody>
                    @foreach($proformaInvoice->proformaInvoiceMachines as $index => $machine)
                        @php
                            ...
                            $unitUsd = (float) ($machine->amount ?? 0);
                            $lineUsd = ($unitUsd * (float) $qty) + (float) ($machine->amc_price ?? 0);
                            $unitPrice = $tplPdfIsLocal ? $unitUsd * $tplPdfLocalMult : $unitUsd;
                            $qtyPrice = $tplPdfIsLocal ? $lineUsd * $tplPdfLocalMult : $lineUsd;
                        ...
                        <tr>
                            ...
                            <td class="unit-price">{{ $tplPdfPrefix }} {{ number_format($unitPrice, 2) }}</td>
                            <td class="total-price">{{ $tplPdfPrefix }} {{ number_format($qtyPrice, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
            ...
            <div class="pricing-section">
                <div class="pricing-table">
                    ...
                    <div class="pricing-row">
                        ...
                        <span class="pricing-value">{{ $tplPdfPrefix }} {{ number_format($totalPIPrice, 2) }}</span>
                    </div>
                    <div class="pricing-row">
                        <span class="pricing-label">GST ({{ $gstPercentage }}%)</span>
                        <span class="pricing-value">{{ $tplPdfPrefix }} {{ number_format($gstAmount, 2) }}</span>
                    </div>
                    <div class="pricing-row total-row">
                        <span class="pricing-label">Final Amount (Including GST)</span>
                        <span class="pricing-value">{{ $tplPdfPrefix }} {{ number_format($finalAmount, 2) }}</span>
                    </div>
                </div>
